  ...
  Element.prototype.init = function (type) {
    var that = this,
      ...
      tab: function () {
        ...
        //Tab切换
        body.off('click', TITLE, call.tabClick).on('click', TITLE, call.tabClick);

        //自适应
        $(window).off('resize', call.tabAuto).on('resize', call.tabAuto);

        //隐藏展开的Tab
        $(document).off('click', call.hideTabMore).on('click', call.hideTabMore);
      }
      //导航菜单
      , nav: function () {
        var ELEM = '.layui-nav',
