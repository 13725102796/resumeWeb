  ...
  var $ = layui.jquery,
    ...
    fixbar: function (options) {
      ...
      var TOP_BAR = 'layui-fixbar-top',
          timer,
          is,
          icon = [options.bar1 === true ? '&#xe606;' : options.bar1 //bar1 - 信息图标
      , options.bar2 === true ? '&#xe607;' : options.bar2 //bar2 - 问号图标
      , '&#xe604;' //置顶
      ],
          dom = $(['<ul class="layui-fixbar">', options.bar1 ? '<li class="layui-icon" lay-type="bar1" style="' + options.bgcolor + '">' + icon[0] + '</li>' : '', options.bar2 ? '<li class="layui-icon" lay-type="bar2" style="' + options.bgcolor + '">' + icon[1] + '</li>' : '', '<li class="layui-icon ' + TOP_BAR + '" lay-type="top" style="' + options.bgcolor + '">' + icon[2] + '</li>', '</ul>'].join('')),
